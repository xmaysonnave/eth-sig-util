...
test('normalize address lower cases', function (t) {
    t.plan(1);
    var initial = '0xA06599BD35921CfB5B71B4BE3869740385b0B306';
    var result = sigUtil.normalize(initial);
...
test('personalSign and recover', function (t) {
    ...
    var msgParams = { data: message };
    var signed = sigUtil.personalSign(privKey, msgParams);
...
test('personalSign and extractPublicKey', function (t) {
    t.plan(1);
    var privKeyHex = '4af1bceebf7f3634ec3cff8a2c38e51178d5d4ce585c52d6043e5e2cc3418bb0';
    var pubKeyHex = '0x9e9e45b2ec5f070b4e26f57c7fedf647afa7a03e894789816fbd12fedc5acd79d0dfeea925688e177caccb8f5e09f0c289bbcfc7adb98d76f5f8c5259478903a';
    var privKey = Buffer.from(privKeyHex, 'hex');
    var message = 'Hello, world!';
    var msgParams = { data: message };
    var signed = sigUtil.personalSign(privKey, msgParams);
...
test('signTypedDataLegacy and recoverTypedSignatureLegacy - single message', function (t) {
    ...
    var signature = sigUtil.signTypedDataLegacy(privKey, msgParams);
    var recovered = sigUtil.recoverTypedSignatureLegacy({ data: msgParams.data, sig: signature });
...
test('signTypedDataLegacy as v1 and recoverTypedSignatureLegacy - single message', function (t) {
    ...
    var msgParams = { data: typedData };
    var signature = sigUtil.signTypedMessage(privKey, msgParams, 'V1');
    var recovered = sigUtil.recoverTypedMessage({ data: msgParams.data, sig: signature }, 'V1');
    t.equal(signature, '0x49e75d475d767de7fcc67f521e0d86590723d872e6111e51c393e8c1e2f21d032dfaf5833af158915f035db6af4f37bf2d5d29781cd81f28a44c5cb4b9d241531b', 'Signature matches test value.');
    t.equal(address, recovered);
    t.end();
});
...
test('typedSignatureHash - single value', function (t) {
    ...
    var hash = sigUtil.typedSignatureHash(typedData);
...
function signatureTest(opts) {
    test(opts.testLabel, function (t) {
        t.plan(2);
        var address = opts.addressHex;
        var privKey = opts.privateKey;
...
function typedSignatureHashThrowsTest(opts) {
    var label = "typedSignatureHash - malformed arguments - " + opts.testLabel;
    test(label, function (t) {
        t.plan(1);
        var argument = opts.argument;
        t.throws(function () {
...
var secretMessage = { data: 'My name is Satoshi Buterin' };
var encryptedData = {
...
test("Getting bob's encryptionPublicKey", function (t) { return __awaiter(void 0, void 0, void 0, function () {
    var result;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                t.plan(1);
                return [4 /*yield*/, sigUtil.getEncryptionPublicKey(bob.ethereumPrivateKey)];
            case 1:
                result = _a.sent();
...
// encryption test
test("Alice encrypts message with bob's encryptionPublicKey", function (t) { return __awaiter(void 0, void 0, void 0, function () {
    var result;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                t.plan(4);
                return [4 /*yield*/, sigUtil.encrypt(bob.encryptionPublicKey, secretMessage, 'x25519-xsalsa20-poly1305')];
            case 1:
                result = _a.sent();
...
test("Alice encryptsSafely message with bob's encryptionPublicKey", function (t) { return __awaiter(void 0, void 0, void 0, function () {
    var VERSION, result;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                t.plan(5);
                VERSION = 'x25519-xsalsa20-poly1305';
                return [4 /*yield*/, sigUtil.encryptSafely(bob.encryptionPublicKey, secretMessage, VERSION)];
            case 1:
                result = _a.sent();
                console.log('RESULT', result);
...
test('Bob decryptSafely message that Alice encryptSafely for him', function (t) { return __awaiter(void 0, void 0, void 0, function () {
    var VERSION, result, plaintext;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                t.plan(1);
            ...
            case 1:
                result = _a.sent();
                plaintext = sigUtil.decryptSafely(result, bob.ethereumPrivateKey);
...
// decryption test
test('Bob decrypts message that Alice sent to him', function (t) {
    t.plan(1);
    var result = sigUtil.decrypt(encryptedData, bob.ethereumPrivateKey);
    t.equal(result, secretMessage.data);
});
test('Decryption failed because version is wrong or missing', function (t) {
    t.plan(1);
    var badVersionData = {
...
test('Decryption failed because nonce is wrong or missing', function (t) {
    t.plan(1);
    // encrypted data
    var badNonceData = {
        version: 'x25519-xsalsa20-poly1305',
...
test('Decryption failed because ephemPublicKey is wrong or missing', function (t) {
    t.plan(1);
    // encrypted data
    var badEphemData = {
        version: 'x25519-xsalsa20-poly1305',
...
test('Decryption failed because cyphertext is wrong or missing', function (t) {
    t.plan(1);
    // encrypted data
    var badEphemData = {
        version: 'x25519-xsalsa20-poly1305',
...
test('signedTypeData', function (t) {
    t.plan(8);
    var typedData = {
        types: {
            EIP712Domain: [
    ...
    var utils = sigUtil.TypedDataUtils;
    var privateKey = ethUtil.sha3('cow');
    var address = ethUtil.privateToAddress(privateKey);
    var sig = sigUtil.signTypedData(privateKey, { data: typedData });
    t.equal(utils.encodeType('Mail', typedData.types), 'Mail(Person from,Person to,string contents)Person(string name,address wallet)');
    t.equal(ethUtil.bufferToHex(utils.hashType('Mail', typedData.types)), '0xa0cedeb2dc280ba39b857546d74f5549c3a1d7bdc2dd96bf881f76108e23dac2');
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct(typedData.primaryType, typedData.message, typedData.types)), '0xc52c0ee5d84264471806290a3f2c4cecfc5490626bf912d01f240d7a274b371e');
    t.equal(ethUtil.bufferToHex(utils.hashStruct('EIP712Domain', typedData.domain, typedData.types)), '0xf2cee375fa42b42143804025fc449deafd50cc031ca257e0b194a650a912090f');
    t.equal(ethUtil.bufferToHex(utils.sign(typedData)), '0xbe609aee343fb3c4b28e1df9e632fca64fcfaede20f02e86244efddf30957bd2');
...
test('signedTypeData with V3 string', function (t) {
    t.plan(8);
    var typedData = {
        types: {
            EIP712Domain: [
    ...
    var privateKey = ethUtil.sha3('cow');
    var address = ethUtil.privateToAddress(privateKey);
    var sig = sigUtil.signTypedMessage(privateKey, { data: typedData }, 'V3');
    t.equal(utils.encodeType('Mail', typedData.types), 'Mail(Person from,Person to,string contents)Person(string name,address wallet)');
    t.equal(ethUtil.bufferToHex(utils.hashType('Mail', typedData.types)), '0xa0cedeb2dc280ba39b857546d74f5549c3a1d7bdc2dd96bf881f76108e23dac2');
    t.equal(ethUtil.bufferToHex(utils.encodeData(typedData.primaryType, typedData.message, typedData.types)), '');
    t.equal(ethUtil.bufferToHex(utils.hashStruct(typedData.primaryType, typedData.message, typedData.types)), '0xc52c0ee5d84264471806290a3f2c4cecfc5490626bf912d01f240d7a274b371e');
...
test('signedTypeData_v4', function (t) {
    t.plan(15);
    var typedData = {
        types: {
            EIP712Domain: [
    ...
    var utils = sigUtil.TypedDataUtils;
    t.equal(utils.encodeType('Group', typedData.types), 'Group(string name,Person[] members)Person(string name,address[] wallets)');
    t.equal(utils.encodeType('Person', typedData.types), 'Person(string name,address[] wallets)');
    t.equal(ethUtil.bufferToHex(utils.hashType('Person', typedData.types)), '0xfabfe1ed996349fc6027709802be19d047da1aa5d6894ff5f6486d92db2e6860');
    t.equal(ethUtil.bufferToHex(utils.encodeData('Person', typedData.message.from, typedData.types)), "0x" + [
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct('Person', typedData.message.from, typedData.types)), '0x9b4846dd48b866f0ac54d61b9b21a9e746f921cefa4ee94c4c0a1c49c774f67f');
    t.equal(ethUtil.bufferToHex(utils.encodeData('Person', typedData.message.to[0], typedData.types)), "0x" + [
        'fabfe1ed996349fc6027709802be19d047da1aa5d6894ff5f6486d92db2e6860',
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct('Person', typedData.message.to[0], typedData.types)), '0xefa62530c7ae3a290f8a13a5fc20450bdb3a6af19d9d9d2542b5a94e631a9168');
    t.equal(utils.encodeType('Mail', typedData.types), 'Mail(Person from,Person[] to,string contents)Person(string name,address[] wallets)');
    t.equal(ethUtil.bufferToHex(utils.hashType('Mail', typedData.types)), '0x4bd8a9a2b93427bb184aca81e24beb30ffa3c747e2a33d4225ec08bf12e2e753');
    t.equal(ethUtil.bufferToHex(utils.encodeData(typedData.primaryType, typedData.message, typedData.types)), "0x" + [
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct(typedData.primaryType, typedData.message, typedData.types)), '0xeb4221181ff3f1a83ea7313993ca9218496e424604ba9492bb4052c03d5c3df8');
    t.equal(ethUtil.bufferToHex(utils.hashStruct('EIP712Domain', typedData.domain, typedData.types)), '0xf2cee375fa42b42143804025fc449deafd50cc031ca257e0b194a650a912090f');
    t.equal(ethUtil.bufferToHex(utils.sign(typedData)), '0xa85c2e2b118698e88db68a8105b794a8cc7cec074e89ef991cb4f5f533819cc2');
    var privateKey = ethUtil.sha3('cow');
    var address = ethUtil.privateToAddress(privateKey);
    t.equal(ethUtil.bufferToHex(address), '0xcd2a3d9f938e13cd947ec05abc7fe734df8dd826');
    var sig = sigUtil.signTypedData_v4(privateKey, { data: typedData });
...
test('signedTypeData_v4 with recursive types', function (t) {
    t.plan(12);
    var typedData = {
        types: {
            EIP712Domain: [
    ...
    var utils = sigUtil.TypedDataUtils;
    t.equal(utils.encodeType('Person', typedData.types), 'Person(string name,Person mother,Person father)');
    t.equal(ethUtil.bufferToHex(utils.hashType('Person', typedData.types)), '0x7c5c8e90cb92c8da53b893b24962513be98afcf1b57b00327ae4cc14e3a64116');
    t.equal(ethUtil.bufferToHex(utils.encodeData('Person', typedData.message.mother, typedData.types)), "0x" + [
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct('Person', typedData.message.mother, typedData.types)), '0x9ebcfbf94f349de50bcb1e3aa4f1eb38824457c99914fefda27dcf9f99f6178b');
    t.equal(ethUtil.bufferToHex(utils.encodeData('Person', typedData.message.father, typedData.types)), "0x" + [
        '7c5c8e90cb92c8da53b893b24962513be98afcf1b57b00327ae4cc14e3a64116',
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct('Person', typedData.message.father, typedData.types)), '0xb852e5abfeff916a30cb940c4e24c43cfb5aeb0fa8318bdb10dd2ed15c8c70d8');
    t.equal(ethUtil.bufferToHex(utils.encodeData(typedData.primaryType, typedData.message, typedData.types)), "0x" + [
        '7c5c8e90cb92c8da53b893b24962513be98afcf1b57b00327ae4cc14e3a64116',
    ...
    t.equal(ethUtil.bufferToHex(utils.hashStruct(typedData.primaryType, typedData.message, typedData.types)), '0xfdc7b6d35bbd81f7fa78708604f57569a10edff2ca329c8011373f0667821a45');
    t.equal(ethUtil.bufferToHex(utils.hashStruct('EIP712Domain', typedData.domain, typedData.types)), '0xfacb2c1888f63a780c84c216bd9a81b516fc501a19bae1fc81d82df590bbdc60');
    t.equal(ethUtil.bufferToHex(utils.sign(typedData)), '0x807773b9faa9879d4971b43856c4d60c2da15c6f8c062bd9d33afefb756de19c');
    var privateKey = ethUtil.sha3('dragon');
    var address = ethUtil.privateToAddress(privateKey);
...
test('signedTypeMessage V4 with recursive types', function (t) {
    t.plan(12);
    var typedData = {
        types: {
            EIP712Domain: [
    ...
    var address = ethUtil.privateToAddress(privateKey);
    t.equal(ethUtil.bufferToHex(address), '0x065a687103c9f6467380bee800ecd70b17f6b72f');
    var sig = sigUtil.signTypedMessage(privateKey, { data: typedData }, 'V4');
    t.equal(sig, '0xf2ec61e636ff7bb3ac8bc2a4cc2c8b8f635dd1b2ec8094c963128b358e79c85c5ca6dd637ed7e80f0436fe8fce39c0e5f2082c9517fe677cc2917dcd6c84ba881c');
});
